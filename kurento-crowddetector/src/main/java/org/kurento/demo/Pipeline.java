...
public class Pipeline {
	...
	public void init() {
		...
		this.rateFilter = new GStreamerFilter.Builder(this.pipe,
				"capsfilter caps=video/x-raw,framerate=15/1").withFilterType(
				FilterType.VIDEO).build();

		this.crowdDetectorFilter = new CrowdDetectorFilter.Builder(this.pipe,
				this.rois).build();
		...
		this.playerEndpoint = new PlayerEndpoint.Builder(this.pipe,
				this.feedUrl).build();
		addPlayerListeners();
		this.playing = true;
	...
	private void addPlayerListeners() {
		...
		this.playerEndpoint.addErrorListener(new EventListener<ErrorEvent>() {
			...
			public void onEvent(ErrorEvent event) {
				synchronized (this) {
					log.error("Error received from Media server"
	...
	private static List<RegionOfInterest> readRoisFromJson(JsonArray loadedRois) {
		...
		for (int j = 0; j < loadedRois.size(); j++) {
			...
			config.setSendOpticalFlowEvent(roi.getAsJsonPrimitive(
					"sendOpticalFlowEvent").getAsBoolean());

			config.setOpticalFlowNumFramesToEvent(roi.getAsJsonPrimitive(
					"opticalFlowNumFramesToEvent").getAsInt());
			config.setOpticalFlowNumFramesToReset(roi.getAsJsonPrimitive(
					"opticalFlowNumFramesToReset").getAsInt());
		...
		this.crowdDetectorFilter
				.addCrowdDetectorDirectionListener(new EventListener<CrowdDetectorDirectionEvent>() {
					...
					public void onEvent(CrowdDetectorDirectionEvent event) {
						...
						log.debug(
								"Direction event detected in roi {} direction {}",
								event.getRoiID(), event.getDirectionAngle());
					}
				});
		...
		this.crowdDetectorFilter
				.addCrowdDetectorFluidityListener(new EventListener<CrowdDetectorFluidityEvent>() {
					...
					public void onEvent(CrowdDetectorFluidityEvent event) {
						...
						log.debug(
								"Fluidity event detected in roi {} percentage {}  level {}",
								event.getRoiID(),
		...
		this.crowdDetectorFilter
				.addCrowdDetectorOccupancyListener(new EventListener<CrowdDetectorOccupancyEvent>() {
					...
					public void onEvent(CrowdDetectorOccupancyEvent event) {
						...
						log.debug(
								"Occupancy event detected in roi {} percentage {} level {}",
								event.getRoiID(),
	...
	public void setFeedUrl(String feedUrl) {
		...
		log.debug("Connecting " + this.webRtcEndpoints.size()
				+ " webrtcendpoints");
	...
	public void addCandidate(IceCandidate e, String session) {
		WebRtcEndpoint endpoint = this.webRtcEndpoints.get(session);
		if (endpoint != null) {
			endpoint.addIceCandidate(e);
		}
	}
}
