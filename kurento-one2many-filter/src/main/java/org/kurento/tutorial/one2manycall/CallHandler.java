...
public class CallHandler extends TextWebSocketHandler {
	final static String DEFAULT_ALVAR_OVERLAY_URI = "http://localhost:8080/img/kurento-logo.png";
	...
	private MasterUserSession masterUserSession;
	...
	private synchronized void master(WebSocketSession session,
		...
		if (masterUserSession == null) {
			masterUserSession = new MasterUserSession(session);

			pipeline = kurento.createMediaPipeline();
			masterUserSession.setWebRtcEndpoint(new WebRtcEndpoint.Builder(
					pipeline).build());
			
			masterUserSession.setArMarkerdetector(new ArMarkerdetector.Builder(
					pipeline).build());
			String alvarOverlayUri = System.getProperty(
					"kms.alvar.overlay.uri", DEFAULT_ALVAR_OVERLAY_URI);
			log.debug("Set ALVAR overlay to " + alvarOverlayUri);
			masterUserSession.getArMarkerdetector().setOverlayImage(alvarOverlayUri);
			
			masterUserSession.getWebRtcEndpoint().connect(
					masterUserSession.getArMarkerdetector());
	...
	private synchronized void viewer(WebSocketSession session,
			JsonObject jsonMessage) throws IOException {
		if (masterUserSession == null
				|| masterUserSession.getArMarkerdetector() == null) {
			JsonObject response = new JsonObject();
			response.addProperty("id", "viewerResponse");
			response.addProperty("response", "rejected");
			response.addProperty("message",
			...
			masterUserSession.getArMarkerdetector().connect(nextWebRtc);
