...
public class HelloWorldRecHandler extends TextWebSocketHandler {
	...
	private final Logger log = LoggerFactory
			.getLogger(HelloWorldRecHandler.class);
	...
	public void handleTextMessage(WebSocketSession session, TextMessage message)
		...
		if (user != null) {
			log.debug("Incoming message from user '{}': {}", user.getId(),
					jsonMessage);
		} else {
			...
			case "onIceCandidate": {
				...
				if (user != null) {
					IceCandidate candidate =
							new IceCandidate(jsonCandidate.get("candidate")
									.getAsString(), jsonCandidate.get("sdpMid")
									.getAsString(), jsonCandidate.get(
	...
	private void start(final WebSocketSession session, JsonObject jsonMessage) {
		try {
			...
			if (repositoryClient != null) {
				...
				String filePath =
						HelloWorldRecApp.REPOSITORY_SERVER_URI + now
								+ RECORDING_EXT;
			...
			RecorderEndpoint recorder =
					new RecorderEndpoint.Builder(pipeline, repoItem.getUrl())
	...
	private void play(UserSession user, final WebSocketSession session,
			JsonObject jsonMessage) {
		try {
			// 0. Repository logic
			...
			PlayerEndpoint player =
					new PlayerEndpoint.Builder(pipeline, itemPlayer.getUrl())
							.build();
			...
			player.addErrorListener(new EventListener<ErrorEvent>() {
				...
				public void onEvent(ErrorEvent event) {
					log.info("ErrorEvent for session '{}': {}",
							session.getId(), event.getDescription());
			...
			player.addEndOfStreamListener(new EventListener<EndOfStreamEvent>() {
				...
				public void onEvent(EndOfStreamEvent event) {
					log.info("EndOfStreamEvent for session '{}'",
							session.getId());
					sendPlayEnd(session, pipeline);
				}
			});
